//60-371 Iterated Prisoner's Dilemma Genetic Algorithm
//Nik Steel
//March 21th 5 PM
//Version used for final data
...
std::istream& operator>>(std::istream & is, Simulation::Parameters& p)
{
   ...
   if (is) {
      ...
      for (size_t i = 0; i < tmp.weights.size(); ++i) {
         assert((tmp.weights[i] >= 0.0) && (tmp.weights[i] <= 1.0));
         sum_of_weights += tmp.weights[i]*1000;
      }
      
      assert(sum_of_weights == 1000 && "The six real-valued weights must sum to 1.0");
      */
      p = std::move(tmp);
   }
   return is;
